...
function gamipress_bp_activity_meta_boxes() {
    ...
    gamipress_add_meta_box(
        ...
        __( 'BuddyPress Member Activity', 'gamipress-buddypress-integration' ),
        array( 'points-type' ),
        array(
            $prefix . 'create_points_award_activity' => array(
                'name' => __( 'Awards activity entries', 'gamipress-buddypress-integration' ),
                'desc' => __( 'Create an activity entry on user\'s profile when they get awarded by points award of this type.', 'gamipress-buddypress-integration' ),
                'type' => 'checkbox',
                'classes' => 'gamipress-switch'
            ),
            $prefix . 'create_points_deduct_activity' => array(
                'name' => __( 'Deducts activity entries', 'gamipress-buddypress-integration' ),
                'desc' => __( 'Create an activity entry on user\'s profile when they get deducted by a points deduct of this type.', 'gamipress-buddypress-integration' ),
                'type' => 'checkbox',
                'classes' => 'gamipress-switch'
            ),
        )
    ...
    gamipress_add_meta_box(
        ...
        array(
            $prefix . 'create_achievement_activity' => array(
                'name' => __( 'Achievement activity entries', 'gamipress-buddypress-integration' ),
                'desc' => __( 'Create an activity entry on user\'s profile when they earn an achievement of this type.', 'gamipress-buddypress-integration' ),
                'type' => 'checkbox',
                'classes' => 'gamipress-switch'
            ),
            $prefix . 'create_step_activity' => array(
                'name' => __( 'Step activity entries', 'gamipress-buddypress-integration' ),
                'desc' => __( 'Create an activity entry on user\'s profile when they complete a step of an achievement of this type.', 'gamipress-buddypress-integration' ),
                'type' => 'checkbox',
                'classes' => 'gamipress-switch'
            ),
        )
    ...
    gamipress_add_meta_box(
        ...
        array(
            $prefix . 'create_rank_activity' => array(
                'name' => __( 'Rank activity entries', 'gamipress-buddypress-integration' ),
                'desc' => __( 'Create an activity entry on user\'s profile when they reach a new rank of this type.', 'gamipress-buddypress-integration' ),
                'type' => 'checkbox',
                'classes' => 'gamipress-switch'
            ),
            $prefix . 'create_rank_requirement_activity' => array(
                'name' => __( 'Rank requirement activity entries', 'gamipress-buddypress-integration' ),
                'desc' => __( 'Create an activity entry on user\'s profile when they complete a rank requirement of a rank of this type.', 'gamipress-buddypress-integration' ),
                'type' => 'checkbox',
                'classes' => 'gamipress-switch'
            ),
        )
...
function gamipress_bp_points_activity_details( $activity, $user_id, $post_id, $trigger_type, $site_id, $args ) {
    ...
    $action_pattern = __( '%1$s earned %2$s', 'gamipress-buddypress-integration' );

    if( $post_type === 'points-deduct' ) {
        $action_pattern = __( '%1$s lost %2$s', 'gamipress-buddypress-integration' );
    }
...
function gamipress_bp_achievement_activity_details( $activity, $user_id, $post_id, $trigger_type, $site_id, $args ) {
    ...
    // Setup the activity details
    $activity = array(
        'action'       => sprintf( __( '%1$s earned the %2$s %3$s', 'gamipress-buddypress-integration' ), bp_core_get_userlink( $user_id ), $singular_name, '<a href="' . get_permalink( $post_id ) . '">' . $post->post_title . '</a>' ),
        'content'      => $content,
        'component'    => 'gamipress',
        'type'         => 'activity_update',
        'primary_link' => get_permalink( $post_id ),
...
function gamipress_bp_step_activity_details( $activity, $user_id, $post_id, $trigger_type, $site_id, $args ) {
    ...
    // Setup the activity details
    $activity = array(
        'action'       => sprintf( __( '%1$s completed the step "%2$s" of the %3$s %4$s', 'gamipress-buddypress-integration' ), bp_core_get_userlink( $user_id ), $post->post_title, $singular_name, '<a href="' . get_permalink( $achievement->ID ) . '">' . $achievement->post_title . '</a>' ),
        'content'      => $content,
        'component'    => 'gamipress',
        'type'         => 'activity_update',
...
function gamipress_bp_rank_activity_details( $activity, $user_id, $post_id, $trigger_type, $site_id, $args ) {
    ...
    // Setup the activity details
    $activity = array(
        'action'       => sprintf( __( '%1$s reached the %2$s %3$s', 'gamipress-buddypress-integration' ), bp_core_get_userlink( $user_id ), $singular_name, '<a href="' . get_permalink( $post_id ) . '">' . $post->post_title . '</a>' ),
        'content'      => $content,
        'component'    => 'gamipress',
        'type'         => 'activity_update',
        'primary_link' => get_permalink( $post_id ),
...
function gamipress_bp_rank_requirement_activity_details( $activity, $user_id, $post_id, $trigger_type, $site_id, $args ) {
    ...
    // Setup the activity details
    $activity = array(
        'action'       => sprintf( __( '%1$s completed the requirement "%2$s" of the %3$s %4$s', 'gamipress-buddypress-integration' ), bp_core_get_userlink( $user_id ), $post->post_title, $singular_name, '<a href="' . get_permalink( $rank->ID ) . '">' . $rank->post_title . '</a>' ),
        'content'      => $content,
        'component'    => 'gamipress',
        'type'         => 'activity_update',
